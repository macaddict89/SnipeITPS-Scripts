...
#When creating a computer model I like to add the default fieldset of Asset with MAC Address.
...
if(([string]::IsNullOrEmpty($asset))){
    ...
    if(([string]::IsNullOrEmpty($modelExists))){
        ...
        $model = New-Model -Name $model -category_id $categoryID -manufacturer_id $manufacturerID -fieldset_id $fieldsetID
        $modelID = $model.id
        #You may want to add more details to a newly created item
    ...
    $asset = New-Asset -Name $computerName -Status_id $statusID -Model_id $modelID -customfields @{"serial" = $wmiBios.SerialNumber; $macaddressField = $wmiNetwork.MACAddress[1]; $processorField = $wmiProcessor.Name; $memoryField = "$($memoryAmount)GB"; $osField = $wmiOS.Caption; $storageField = "$($wmiDisks.model + " " + $wmiDisks.GB)GB"}
    $tag = $asset.asset_tag
    $assetID = $asset.id
